...
inline DnaSequence::DnaSequence(const string seq) : m_seq(NULL) , m_length(seq.length()){
    ...
    try{
        ...
        delete [] m_seq;
        throw std::invalid_argument("invalid argument, please try again");;
    }
}
...
inline DnaSequence::operator std::string() const
{
    std::string _ = "";
    for (size_t i = 0; i < m_length; i++)
    {
...
/*getSeq*/
inline const DnaSequence::Nucleotide* DnaSequence::getSeq() const
{
	return m_seq;
}

/*getSeqAsString*/
inline const std::string DnaSequence::getSeqAsString() const {
    std::string _ = "";
...
/*slice*/
inline std::string & DnaSequence::slice(size_t start , size_t end)
{
    string myString = "";
